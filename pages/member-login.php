<?php
session_start();
require_once __DIR__ . '/../includes/MemberLogin.php';
...
$action = $_GET['action'] ?? 'login'; // login, register, verify
    ...
    if (!isset($_POST['csrf_token']) || !verifyCsrfToken($_POST['csrf_token'])) {
        ...
        if ($action === 'register') {
            ...
            if ($password !== $confirmPassword) {
                ...
                $result = $memberLogin->register($email, $password, $firstName, $lastName, $phone, $companyName, $address);
                if ($result) {
                    $success[] = $memberLogin->getSuccess()[0];
                    $_SESSION['pending_verification_email'] = $email;
                    $_SESSION['verification_token'] = $result['verification_token'];
                    $action = 'verify'; // Doğrulama sayfasına yönlendir
                } else {
                    $errors = $memberLogin->getErrors();
                }
            }
            ...
            if ($memberLogin->verifyEmail($email, $token)) {
                $success = $memberLogin->getSuccess();
                $action = 'login'; // Login sayfasına yönlendir
            } else {
                $errors = $memberLogin->getErrors();
            }
        } else {
            ...
            if ($memberLogin->login($email, $password)) {
                $success = $memberLogin->getSuccess();
...
<body>
    <div class="container">
        <div class="header">
            <h1>🚚 Yuknet Üye</h1>
            <p>Nakliye Sağlayıcıları İçin Platform</p>
        </div>
        ...
        <form id="registerForm" method="POST" action="?action=register" class="<?php echo $action === 'register' ? '' : 'hidden'; ?>">
            <div class="form-row">
                <div class="form-group">
                    <label for="reg-first-name">Ad</label>
                    <input type="text" id="reg-first-name" name="first_name" required placeholder="Ad">
                </div>

                <div class="form-group">
                    <label for="reg-last-name">Soyad</label>
                    <input type="text" id="reg-last-name" name="last_name" required placeholder="Soyad">
                </div>
            </div>
            ...
            <div class="form-group">
                <label for="reg-company">Şirket Adı</label>
                <input type="text" id="reg-company" name="company_name" placeholder="Şirket adınız (isteğe bağlı)">
            </div>

            <div class="form-group">
                <label for="reg-address">Adres</label>
                <textarea id="reg-address" name="address" placeholder="Adres (isteğe bağlı)" rows="3"></textarea>
            </div>
        ...
        <!-- Email Doğrulama Formu -->
        <form id="verifyForm" method="POST" action="?action=verify" class="<?php echo $action === 'verify' ? '' : 'hidden'; ?>">
            <div class="alert alert-success">
                ✓ Kaydınız başarılı! Lütfen email adresinize gönderilen aktivasyon kodunu giriniz.
            </div>

            <div class="form-group">
                <label for="verify-token">Aktivasyon Kodu</label>
                <input type="text" id="verify-token" name="verification_token" required placeholder="Aktivasyon kodunuzu giriniz">
                <span class="badge">Email: <?php echo escapeHTML($_SESSION['pending_verification_email'] ?? ''); ?></span>
            </div>

            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">

            <button type="submit" class="submit-btn">Email'i Doğrula</button>
        </form>
    </div>
